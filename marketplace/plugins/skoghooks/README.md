...
# ... hook-specific logic ...
# log to <runtime_dir>/{name}.jsonl via append_jsonl(), see get_runtime_dir()
...
New hooks are created from the repository root with the scaffolder, which
generates a script from `templates/hook_template.py` and wires it into
`hooks/hooks.json`:

```shell
uv run scripts/new_hook.py <Event> <name> [--matcher "Write|Edit"] [--flags="--my-flag"]
uv run scripts/test_hooks.py    # pipe tests/payloads/<Event>.json through every wired hook
```

See the `/new-hook` skill (`.claude/skills/new-hook/SKILL.md`) for the full
scaffold → implement → test → validate → document workflow.
