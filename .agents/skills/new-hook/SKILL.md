...
```shell
Codex plugin validate ./plugins/skoghooks
Codex plugin validate .
```

If the `Codex` CLI is unavailable, at minimum check hooks.json parses:
`python3 -c "import json; json.load(open('plugins/skoghooks/hooks/hooks.json'))"`.

## 5. Document

- Add the hook to the flag table and event table in `AGENTS.md` and
  `plugins/skoghooks/README.md`.
- If behavior is non-obvious (blocking rules, external calls, env vars), add a
  bullet under "Non-Obvious Behaviours" in `AGENTS.md`.
