...
**skogai/harness** (npm: `skogharness`, bin: `harness`/`skogharness`) is a small, depth-focused multi-agent skill pack and project-level agent config manager — a hard fork of `agent-starter` adapted to fit the skogai workflow. It ships one shared set of hand-written skills and generates native project files for two agent targets:
...
Users run `npx skogharness@latest` and choose targets with `--agent claude|codex|all`.
...
npm run lint                             # eslint over src/ test/ bin/ templates/.claude/utils/
```

CI (`.github/workflows/ci.yml`) uses Bun 1.3.11 (`bun install --frozen-lockfile`, `bun run lint`, `bun test`, `bun audit`, gitleaks, CodeQL). npm works fine for local root scripts; `packageManager` is pinned to bun.
...
`src/agents.js` defines the two agent targets and resolves `--agent` values (including the `all` alias) into a deduped target list; unknown targets throw.

**One skill source, generated targets**: The canonical skill source is `templates/.claude/skills/<skill>/skill.md` (or `SKILL.md`). `src/utils/copy.js` is the adapter layer that generates the other target at install time: it parses frontmatter, normalizes descriptions ("Claude needs" → "the agent needs"), and writes Codex `SKILL.md` files plus a generated root `AGENTS.md` skill index. Never hand-maintain per-agent copies of a skill. Static (non-generated) README template for Codex lives at `templates/codex/`.
...
- `src/mcps.js` — the MCP catalog (github, neon, stripe, resend) and per-target config rendering: `.mcp.json` (Claude), `.codex/config.toml`. Secrets stay as `${VAR}` references, never resolved into generated files.
...
**Plugin marketplace**: `.claude-plugin/marketplace.json` declares this repo as a Claude Code plugin marketplace, separate from the npm package. It currently lists only `toon-formatter`; update it deliberately when a skill should be independently installable as a plugin — don't assume every shipped skill belongs there.
